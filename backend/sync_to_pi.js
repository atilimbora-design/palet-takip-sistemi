...
// Wait, we might not have axios installed in backend folder. Let's use http or check package.json.
// The backend uses express, sqlite3... Let's use vanilla http to be safe, or fetch if node 18+.
// Node version on PC is likely recent. Let's try native fetch.
...
const PI_URL = 'http://192.168.1.104:3000/api/sync';

async function syncToPi() {
    console.log(`Preparing to sync ${jsonData.data.length} records to ${PI_URL}...`);
    ...
    const selectedForTemp = new Set(validIndices.slice(0, 15)); // 15 items with temp
